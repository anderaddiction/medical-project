    ...
    <div class="col-md-4">
        <div class="mb-3">
            ...
            <input type="text" name="name" id="name" class="form-control required" placeholder="{{ __('Enter role') }}"
                value="{{ old('name', $medical_history->name) }}">
    ...
    <div class="col-md-4">
        <div class="mb-3">
            ...
            <input type="text" name="display_name" id="display_name" class="form-control required"
                placeholder="{{ __('Enter display role') }}" value="{{ old('display_name', $medical_history->display_name) }}">
        </div>
    </div>
    <div class="col-md-4">
        <div class="mb-3">
            ...
            <select class="form-select required" name="status">
                ...
                <option value="1" {{ old('status', $medical_history->status) == '1' ? 'selected' : '' }}>@lang('Active')
                </option>
                <option value="0" {{ old('status', $medical_history->status) == '0' ? 'selected' : '' }}>@lang('Inactive')
                </option>
            </select>
            <small class="feedback text-danger">
                {{ $errors->first('status') }}
...
<div class="col-md-12">
    <div class="mb-3">
        ...
        <textarea type="text" cols="10" rows="5" class="form-control required" placeholder="{{ __('Enter note') }}"
            id="note" name="note" value="{{ old('note', $medical_history->note) }}">{{ old('note', $medical_history->note) }}</textarea>
    </div>
</div>
<div class="mt-4">
