    ...
    <div class="col-md-6">
        <div class="mb-3">
            ...
            <select class="form-select required" name="status">
                ...
                <option value="1" {{ old('status', $continent->status) == '1' ? 'selected' : '' }}>@lang('Active')
                </option>
                <option value="0" {{ old('status', $continent->status) == '0' ? 'selected' : '' }}>@lang('Inactive')
                </option>
            </select>
            <small class="feedback text-danger">
                {{ $errors->first('status') }}
...
<div class="col-md-12">
    <div class="mb-3">
        ...
        <textarea type="text" cols="10" rows="5" class="form-control required" placeholder="{{ __('Enter note') }}"
            id="note" name="note" value="{{ old('note', $continent->note) }}">{{ old('note', $continent->note) }}</textarea>
    </div>
</div>
<div class="mt-4">
